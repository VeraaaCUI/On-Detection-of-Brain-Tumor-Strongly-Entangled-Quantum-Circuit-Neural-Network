...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model4.to(device)
model4.load_state_dict(torch.load("q8zz.pt"))
...
for epoch in range(epochs):
    ...
    with open("training_performance_q8_zz.txt", "a") as file:
        file.write(f"{epoch+1}, {avg_train_loss:.4f}%\n")

    print(f"Epoch: {epoch+1}, Training Loss: {avg_train_loss:.4f}")
    if epoch > 2:
        if loss_list[-1] < min(loss_list[:-1]):
            torch.save(model4.state_dict(), "q8zz.pt")
            print("Saved")
...
with open("model_performance_q8zz.txt", "w") as file:
