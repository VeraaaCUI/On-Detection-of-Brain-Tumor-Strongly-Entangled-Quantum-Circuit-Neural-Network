...
optimizer = optim.NAdam(model4.parameters(), lr=0.00001)
...
for epoch in range(epochs):
    ...
    with open("training_performance_q2zz.txt", "a") as file:
    ...
    if epoch > 2:
        if loss_list[-1] < min(loss_list[:-1]):
            torch.save(model4.state_dict(), "q2zz.pt")
            print("Saved")
...
model4.load_state_dict(torch.load("q2zz.pt"))
...
with open("model_performance_q2zz-test.txt", "w") as file:
