...
def create_qnn():
    ...
    # Adjusted feature map for 4 qubits
...
model4.load_state_dict(torch.load("q4.pt"))
...
for epoch in range(epochs):
    ...
    with open("training_performance_q4.txt", "a") as file:
    ...
    if epoch > 2:
        if acc_list[-1] > min(acc_list[:-1]):
            torch.save(model4.state_dict(), "q4.pt")
            print("Saved")
...
with open("model_performance_q4.txt", "w") as file:
