...
for epoch in range(epochs):
    ...
    with open("training_performance_q2.txt", "a") as file:
    ...
    if epoch > 2:
        if loss_list[-1] < min(loss_list[:-1]):
            torch.save(model4.state_dict(), "q2.pt")
            print("Saved")
...
model4.load_state_dict(torch.load("q2.pt"))
...
with open("model_performance_q2.txt", "w") as file:
